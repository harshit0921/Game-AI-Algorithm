...
void AIPlay(int[][] board, boolean whiteTurn, ArrayList<Move> legalMoves) {
  ...
  minmax(newBoard, whiteTurn, maxDepth, currentDepth);
...
float minmax(int[][] lboard, boolean whiteTurn, int maxDepth, int currentDepth)
{
  ...
  for(int i=0; i<legalMoves.size(); i++)
  {
    ...
    scores.add(minmax(newBoard, !whiteTurn, maxDepth, currentDepth+1));
  }
